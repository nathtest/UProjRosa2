#pragma once
#include "CoreMinimal.h"
#include "UiControllerAccessor.h" //CROSS-MODULE INCLUDE V2: -ModuleName=GameBaseSystem -ObjectName=UiControllerAccessor -FallbackName=UiControllerAccessor
#include "G01WalletUiControllerAccessor.generated.h"

UCLASS(Blueprintable, ClassGroup=Custom, meta=(BlueprintSpawnableComponent))
class RS2RE_API UG01WalletUiControllerAccessor : public UUiControllerAccessor {
    GENERATED_BODY()
public:
    UG01WalletUiControllerAccessor(const FObjectInitializer& ObjectInitializer);

    UFUNCTION(BlueprintCallable)
    void SetEventWalletCalcCrown(int32 InValue, bool InIsEnd);
    
    UFUNCTION(BlueprintCallable)
    void SetEventWalletActive(bool InIsActive);
    
};

